// next
import Image from 'next/image';

// material-ui
import { Container, Grid, Typography } from '@mui/material';

// third party
import { motion } from 'framer-motion';

// project import
import MainCard from 'components/MainCard';

// assets
const imgfeature1 = '/assets/images/landing/img-feature1.svg';
const imgfeature2 = '/assets/images/landing/img-feature2.svg';
const imgfeature3 = '/assets/images/landing/img-feature3.svg';

// ==============================|| LANDING - FEATURE PAGE ||============================== //

const FeatureBlock = () => (
  <Container>
    {/* <Grid container alignItems="center" justifyContent="center" spacing={2} sx={{ mt: { md: 15, xs: 2.5 }, mb: { md: 10, xs: 2.5 } }}> */}
    <Grid container alignItems="center" justifyContent="center" spacing={2} sx={{ mt: { md: 8, xs: 2.5 }, mb: { md: 10, xs: 2.5 } }}>
      <Grid item xs={12}>
        <Grid container spacing={1} justifyContent="center" sx={{ mb: 4, textAlign: 'center' }}>
          <Grid item sm={10} md={6}>
            <Grid container spacing={1} justifyContent="center">
{/*               <Grid item xs={12}>
                <Typography variant="subtitle1" color="primary">
                  Mantis nailed it!
                </Typography>
              </Grid> */}
              <Grid item xs={12}>
                <Typography variant="h2" sx={{ mb: 2 }}>
                  Why {process.env.SITE_NAME}?
                </Typography>
              </Grid>
              <Grid item xs={12}>
                <Typography variant="body1">
                  Get easy to digest economic information that is searchable and presentable.
                </Typography>
              </Grid>
            </Grid>
          </Grid>
        </Grid>
      </Grid>
      <Grid item xs={12} sm={6} md={4}>
        <motion.div
          initial={{ opacity: 0, translateY: 550 }}
          animate={{ opacity: 1, translateY: 0 }}
          transition={{
            type: 'spring',
            stiffness: 150,
            damping: 30,
            delay: 0.2
          }}
        >
          <MainCard contentSX={{ p: 3 }}>
            <Grid container spacing={1}>
              <Grid item xs={12}>
                <Image src={imgfeature1} alt="feature" layout="fixed" width={48} height={48} />
              </Grid>
              <Grid item xs={12}>
                <Typography variant="h5" sx={{ fontWeight: 600, mt: 2 }}>
                  Professional Design
                </Typography>
              </Grid>
              <Grid item xs={12}>
                <Typography variant="body1" color="secondary">
                  Professional grade user interface for any kind of research. 
                </Typography>
              </Grid>
            </Grid>
          </MainCard>
        </motion.div>
      </Grid>
      <Grid item xs={12} sm={6} md={4}>
        <motion.div
          initial={{ opacity: 0, translateY: 550 }}
          animate={{ opacity: 1, translateY: 0 }}
          transition={{
            type: 'spring',
            stiffness: 150,
            damping: 30,
            delay: 0.4
          }}
        >
          <MainCard contentSX={{ p: 3 }}>
            <Grid container spacing={1}>
              <Grid item xs={12}>
                <Image src={imgfeature2} alt="feature" layout="fixed" width={48} height={48} />
              </Grid>
              <Grid item xs={12}>
                <Typography variant="h5" sx={{ fontWeight: 600, mt: 2 }}>
                  Flexible Data
                </Typography>
              </Grid>
              <Grid item xs={12}>
                <Typography variant="body1" color="secondary">
                  Multiple charts allows for different perspectives on a set of data. 
                </Typography>
              </Grid>
            </Grid>
          </MainCard>
        </motion.div>
      </Grid>
      <Grid item xs={12} sm={6} md={4}>
        <motion.div
          initial={{ opacity: 0, translateY: 550 }}
          animate={{ opacity: 1, translateY: 0 }}
          transition={{
            type: 'spring',
            stiffness: 150,
            damping: 30,
            delay: 0.6
          }}
        >
          <MainCard contentSX={{ p: 3 }}>
            <Grid container spacing={1}>
              <Grid item xs={12}>
                <Image src={imgfeature3} alt="feature" layout="fixed" width={48} height={48} />
              </Grid>
              <Grid item xs={12}>
                <Typography variant="h5" sx={{ fontWeight: 600, mt: 2 }}>
                  Effective Explanations 
                </Typography>
              </Grid>
              <Grid item xs={12}>
                <Typography variant="body1" color="secondary">
                  Confused? Check out the explanations next to the content you are viewing for a deeper understanding. 
                </Typography>
              </Grid>
            </Grid>
          </MainCard>
        </motion.div>
      </Grid>
    </Grid>
  </Container>
);

export default FeatureBlock;
